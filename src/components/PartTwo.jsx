...
function PartTwo ()  {
  ...
  useEffect(() => {
    const handleScroll = () => {
      const paragraphElement = document.querySelector('.paragraph-1');
      if (paragraphElement) {
        const rect = paragraphElement.getBoundingClientRect();
        const isVisible = rect.top < window.innerHeight && rect.bottom >= 0;
        setIsParagraphVisible(isVisible);
      }

      const noteElement = document.querySelector('.note-one');
      if (noteElement) {
        const rect = noteElement.getBoundingClientRect();
        const isVisible = rect.top < window.innerHeight && rect.bottom >= 0;
        setIsNoteVisible(isVisible);
      }
    };
  ...
  return (
    ...
    <div className="date-time">
      ...
      <div className="w-layout-blockcontainer container-3 w-container">
        ...
        <p className={`paragraph-1 ${isParagraphVisible ? 'active' : ''}`}>It's such a long time, we haven't met.
        ...
        <p className={`paragraph-1 ${isParagraphVisible ? 'active' : ''}`}>We'll get married! Finally!
        <br></br>The wait is really over.
        <br></br>Pleas come to meet us, be our guest.
        <br></br>I belive that we miss each other.</p>
        
        <p className={`paragraph-1 ${isParagraphVisible ? 'active' : ''}`}>We're looking forward to sharing
        <br></br>our good time and the beginning
        <br></br>of the next chapter with you all.</p>
        
      </div>   

        <div className={`note-one ${isNoteVisible ? 'active' : ''}`}>PS. Please come with the way you are in
